@include('.navbar.main')
<form class="max-w-md mx-auto w-1/3 text-black" action="{{route('classes.update',[$school_id,$class_id])}}" method="post" enctype="multipart/form-data">
    @method('put')
    @csrf

    <div class="relative z-0 w-full mb-5 group">
        <input type="text" name="class_name" id="class_name" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" "
               required value="{{old('class_name',$class_data->name)}}"/>
    ...
    <div class="relative z-0 w-full mb-5 group">
        <input type="text" name="desc" id="desc" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" "
               required value="{{old('desc',$class_data->desc)}}"/>
    ...
    <div class="grid md:grid-cols-2 md:gap-6">
        <div class="relative z-0 w-full mb-5 group">
            <input type="number" name="slots" id="slots" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" "
                   required value="{{old('slots',$class_data->slots)}}"/>
            ...
            @if($errors->get('slots'))
                @foreach($errors->get('slots') as $error)
                    <li>{{$error}}</li>
                @endforeach
            @endif
        </div>
        <div class="relative z-0 w-full mb-5 group">
            ...
            <select id="school_type" name="school_type" required class="block py-2.5 px-0 w-full text-sm text-gray-500 bg-transparent border-0 border-b-2 border-gray-200 appearance-none dark:text-gray-400 dark:border-gray-700 focus:outline-none focus:ring-0 focus:border-gray-200 peer">
                ...
                @foreach($schools_types as $school_type)
                    <option value="{{$school_type->id}}" @if($school_type->id==$class_data->school_type) selected @endif>{{$school_type->name}}</option>
                @endforeach
            </select>
            @if($errors->get('city'))
    ...
    <div class="grid md:grid-cols-2 md:gap-6">
        <div class="relative z-0 w-full mb-5 group">
            ...
            <select id="subject1" name="subject1" required class="block py-2.5 px-0 w-full text-sm text-gray-500 bg-transparent border-0 border-b-2 border-gray-200 appearance-none dark:text-gray-400 dark:border-gray-700 focus:outline-none focus:ring-0 focus:border-gray-200 peer">
                ...
                @foreach($subjects as $subject)
                    <option value="{{$subject->id}}" @if($subject->id==$class_data->subject1) selected @endif>{{$subject->name}}</option>
                @endforeach
            </select>
            @if($errors->get('city'))
                @foreach($errors->get('city') as $error)
        ...
        <div class="relative z-0 w-full mb-5 group">
            ...
            <select id="subject2" name="subject2" required class="block py-2.5 px-0 w-full text-sm text-gray-500 bg-transparent border-0 border-b-2 border-gray-200 appearance-none dark:text-gray-400 dark:border-gray-700 focus:outline-none focus:ring-0 focus:border-gray-200 peer">
                ...
                @foreach($subjects as $subject)
                    <option value="{{$subject->id}}" @if($subject->id==$class_data->subject2) selected @endif>{{$subject->name}}</option>
                @endforeach
            </select>
            @if($errors->get('city'))
                @foreach($errors->get('city') as $error)
